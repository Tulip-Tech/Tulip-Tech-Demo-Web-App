...
import { PLAYLIST_LIMIT, PLAYLIST_LIMIT_LIVE } from '#src/config';
...
export const PlaylistContainer = ({ playlistId, type, onPlaylistUpdate, style, children, showEmpty = false }: Props): JSX.Element | null => {
...
export const PlaylistContainerLive = ({ playlistId, type, onPlaylistUpdate, style, children, showEmpty = false }: Props): JSX.Element | null => {
  const isAlternativeShelf = PersonalShelves.some((shelfType) => shelfType === type);
  const {
    isLoading,
    error,
    data: fetchedPlaylist = { title: '', playlist: [] },
  } = usePlaylist(playlistId, { page_limit: PLAYLIST_LIMIT_LIVE.toString() }, !isAlternativeShelf, true);
  let playlist = fetchedPlaylist;
